...
regForm.addEventListener('submit', (event) => {
    ...
    if (checkName(name) && checkName(surname) && checkLogin(login) && checkPassw(password) && checkEmail(email) && checkDate(dob)) {
        let now = new Date;
        let regtime = `${now.getFullYear()}/${now.getMonth()+1}/${now.getDate()} ${now.getHours()}:${now.getMinutes()}`;
        sendRequest(`http://localhost:8080/reg/?name=${name}&surname=${surname}&login=${login}&password=${password}&email=${email}&dob=${dob}&regtime=${regtime}`);
